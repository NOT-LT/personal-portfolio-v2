...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-16 sm:py-[100px] bg-bg-secondary" ref={sectionRef}>
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {PROJECTS.map((p, i) => (
            <SpotlightCard
              key={i}
              className="group reveal opacity-0 translate-y-[18px] transition-[opacity,transform,border-color] duration-[550ms] [&.in]:opacity-100 [&.in]:translate-y-0 bg-surface border border-line rounded-[10px] hover:border-line-accent"
            ...
            >
              <div className="relative z-[1] p-7 flex flex-col gap-4 h-full">
                ...
                <div className="flex flex-wrap gap-2 pt-2 mt-auto border-t border-line">
                  {p.tags.map((t, j) => (
                    <span key={j} className="font-mono text-[0.68rem] tracking-[0.05em] text-dim">{t}</span>
                  ))}
                </div>
              </div>
            </SpotlightCard>
