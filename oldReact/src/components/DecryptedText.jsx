        ...
        if (sequential) {
          if (direction === 'forward') {
            if (prev.size < text.length) {
              ...
              const newR = new Set(prev); newR.add(next)
              setDisplayText(shuffleText(text, newR))
              return newR
            }
          ...
          if (direction === 'reverse') {
            if (pointerRef.current < orderRef.current.length) {
              ...
              const newR = new Set(prev); newR.delete(idx)
              setDisplayText(shuffleText(text, newR))
              if (newR.size === 0) { clearInterval(interval); setIsAnimating(false); setIsDecrypted(false) }
              return newR
            }
            clearInterval(interval); setIsAnimating(false); setIsDecrypted(false); return prev
          }
        } else {
          if (direction === 'forward') {
    ...
    <motion.span className={parentClassName} ref={containerRef} style={styles.wrapper} {...animateProps} {...props}>
      ...
      <span aria-hidden="true">
        {displayText.split('').map((char, index) => (
          <span key={index} className={revealedIndices.has(index) || (!isAnimating && isDecrypted) ? className : encryptedClassName}>
            {char}
          </span>
        ))}
      </span>
